...
const SlideToggle = ({
  ...
  const [expanded, setexpanded] = React.useState(props.expanded ? props.expanded : defaults.expanded);
  ...
  const handleAnimation = React.useCallback(time => {
    ...
    if (expanded) refElement.current.style.height = `${(mainRef.current.height - trn).toFixed(2)}px`;else refElement.current.style.height = `${trn.toFixed(2)}px`;
    if (mainRef.current.requestRef < duration) {
      requestAnimationFrame(handleAnimation);
    } else {
      if (expanded) refElement.current.style.height = `0px`;else refElement.current.style.height = `auto`;
  ...
  return /*#__PURE__*/React__default.createElement("div", {
    className: expanded ? 'active slide-toggle' : 'slide-toggle'
  }, /*#__PURE__*/React__default.cloneElement(props.trigger, {
    onClick: slideElement
...
//# sourceMappingURL=react-smooth-slide-toggle.cjs.development.js.map
